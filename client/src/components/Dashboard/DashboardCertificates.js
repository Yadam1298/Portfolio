...
const DashboardCertificates = () => {
    ...
    const addCertificate = () => {
        setCertificates([
            ...certificates,
            { title: "", organization: "", date: "", link: "", logo: "", isEditing: true },
        ]);
    };
    ...
    const saveCertificate = async (index) => {
        ...
        try {
            if (cert._id) {
                ...
                const res = await axios.put(`http://localhost:5000/api/certificates/${cert._id}`, cert);
                const updated = [...certificates];
                updated[index] = { ...res.data, isEditing: false };
                setCertificates(updated);
            } else {
                // Create new certificate
                const res = await axios.post("http://localhost:5000/api/certificates", cert);
                const updated = [...certificates];
                updated[index] = { ...res.data, isEditing: false };
                setCertificates(updated);
            }
                        ...
                        {cert.isEditing ? (
                            <>
                                ...
                                <div className="btn-group">
                                    <button className="save-btn" onClick={() => saveCertificate(index)}>
                                        Save
                                    </button>
                                    <button className="cancel-btn" onClick={() => toggleEdit(index)}>
                                        Cancel
                                    </button>
                                </div>
                            </>
                            ...
                            <>
                                ...
                                <div className="btn-group">
                                    ...
                                    <button className="delete-btn" onClick={() => deleteCertificate(index)}>
                                        Delete
                                    </button>
                                </div>
                            </>
